...
const GamePlayPage: React.FC = () => {
  ...
  const handleIframeMessage = useCallback(
    async (event: MessageEvent) => {
      ...
      if (type === "CLOUD_BRIDGE_READY") {
        console.log(
          "[GamePlayPage] ✅ Received CLOUD_BRIDGE_READY from iframe."
        );
        ...
        if (error) {
          console.error(
            "[GamePlayPage] ❌ Error saving game data to Supabase:",
            error
          );
          ...
          setTimeout(
            () =>
              setSyncStatus((s) =>
                s === SyncStatus.Synced ? SyncStatus.Ready : s
              ),
            2000
          );
        }
  ...
  useEffect(() => {
    const initializeSave = async () => {
      if (
        ...
        console.log(
          `[GamePlayPage] ⬇️ Bridge is ready. Sending INIT_SAVE_DATA to iframe.`
        );
  ...
  useEffect(() => {
    if (user && game) {
        // Initial log with 0 seconds to mark as "played"
        logPlaytime(user.id, game.id, 0); 
        
        // Set up the interval to log playtime every minute
        playtimeIntervalRef.current = setInterval(() => {
            logPlaytime(user.id, game.id, PLAYTIME_UPDATE_INTERVAL_MS / 1000);
  ...
  const renderSyncStatus = () => {
    ...
    switch (syncStatus) {
      ...
      case SyncStatus.Error:
        ...
        break;
      default:
        icon = <FiLoader className={styles.spinner} />;
        break;
    }
    return (
      <div className={`${styles.syncStatusIndicator} ${statusClass}`}>
        {icon} {syncStatus}
      </div>
    );
  };
