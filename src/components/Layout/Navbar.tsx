...
const Navbar: React.FC<NavbarProps> = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      const gameSearchItems: SearchItem[] = (gameData as any[])
        .filter(isGame)
        .map((game: Game) => ({
          id: game.id,
          name: game.name,
      ...
      const toolSearchItems: SearchItem[] = (toolData as any[])
        .filter(isTool)
        .map((tool: Tool) => ({
          id: tool.id,
          name: tool.name,
      ...
      const results = allSearchableData
        .filter((item) => {
          ...
          if (item.type === "user") {
            ...
            const usernameMatch = item.rawData.username
              ?.toLowerCase()
          ...
          const tagsMatch = item.rawData.tags
            ? item.rawData.tags.some((tag: string) =>
                tag?.toLowerCase()?.includes(lowerCaseQuery)
              )
            : false;
          const descriptionMatch = item.rawData.description
      ...
      <div className={styles.rightSection}>
        ...
        {authLoading ? (
          ...
          <Link href="/account">
            <Image
              src={`${
                userProfile.avatar_url || "/logos/default-avatar.png"
              }?v=${profileVersion}`}
